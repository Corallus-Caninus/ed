...
void
set_current_addr(const int addr)
{
  current_addr_ = addr;
}
...
static void
link_nodes(line_t *const prev, line_t *const next)
{
  prev->q_forw = next;
  next->q_back = prev;
}
...
static void
insert_node(line_t *const lp, line_t *const prev)
{
...
static void
add_line_node(line_t *const lp, const int addr)
{
...
bool
append_lines(const char **const ibufpp, const int addr, const bool isglobal)
{
  int size = 0;
  undo_t *up = 0;
  current_addr_ = addr;
...
bool
copy_lines(const int first_addr, const int second_addr, const int addr)
{
...
bool
delete_lines(const int from, const int to, const bool isglobal)
{
  line_t *n, *p;
...
//TODO: we really search the entire list on each address..? this could at least be kept statefully in the file since linux doesnt allow two processes to mutate a file
int
get_line_node_addr(const line_t *const lp)
{
  ...
  while (p != lp && (p = p->q_forw) != &buffer_head)
    ++addr;
...
char *
get_sbuf_line(const line_t *const lp)
{
  static char *buf = 0;
  static int bufsz = 0;
  int len;
...
bool
join_lines(const int from, const int to, const bool isglobal)
{
  static char *buf = 0;
  static int bufsz = 0;
  int size = 0;
  ...
  while (bp != ep)
    {
      ...
      if (!s || !resize_buffer(&buf, &bufsz, size + bp->len))
	return false;
      memcpy(buf + size, s, bp->len);
      size += bp->len;
      bp = bp->q_forw;
    }
  if (!resize_buffer(&buf, &bufsz, size + 2))
    return false;
  memcpy(buf + size, "\n", 2);
  size += 2;
...
bool
yank_lines(const int from, const int to)
{
  line_t *const ep = search_line_node(inc_addr(to));
  line_t *bp = search_line_node(from);
...
undo_t *
push_undo_atom(const int type, const int from, const int to)
{
  disable_interrupts();
