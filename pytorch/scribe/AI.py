    ...
    if chunk_size > 0:
      for i in range(0, num_tokens - grad_vector_size, chunk_size):
        ...
        if cache is not None:
          ...
          with torch.no_grad(): # Keep no_grad context for forward passes in the loop
            outputs = model(input_ids=cur_input_ids, attention_mask = cur_attention_mask  , labels = cur_input_ids, cache_params = cache, use_cache=True,  cache_position=torch.tensor([i]).unsqueeze(1))
