      ...
      for i in range(0, num_tokens - grad_vector_size, chunk_size):
        ...
        if cache is not None:
          with torch.no_grad(): # Keep no_grad context for forward passes in the loop
            outputs = model(input_ids=cur_input_ids, attention_mask = cur_attention_mask  , labels = cur_input_ids, cache_params = cache,use_cache = True, cache_position=torch.tensor([i]).unsqueeze(0))
#            outputs = model(input_ids=cur_input_ids, attention_mask = cur_attention_mask  , labels = cur_input_ids,  use_cache=True)
    ...
    #      outputs.loss.backward()
      ...
      gc.collect()
      torch.cuda.empty_cache()
      outputs = model(input_ids[:, -grad_vector_size:1+ (-grad_vector_size//2) ], attention_mask=attention_mask[:, -grad_vector_size:1+ (-grad_vector_size//2) ],labels = input_ids[:, -grad_vector_size:1+ (-grad_vector_size//2) ], use_cache=True, cache_params=cache, cache_position=torch.tensor([i]).unsqueeze(0))
