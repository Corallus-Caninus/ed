...
if os.path.exists(filename): # Load model weights and optimizer history
    ...
    model = AutoModelForCausalLM.from_pretrained(model_id, ignore_mismatched_sizes=True, gradient_checkpointing=True).to("cuda") # Load initial weights using config, ignoring size mismatches
