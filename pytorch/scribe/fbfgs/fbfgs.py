...
def _strong_wolfe(
    ...
    if abs(gtd_new) < abs(gtd) and f_new < f_best  and done != True  and f_new == f_new and c1 > (abs(gtd_new) - abs(gtd))/(f_new - f) > 0:
      success = True
      stall_wolfe = 0
      t_best = t
...
#        if abs(gtd_new) < abs(gtd) and ( abs(abs(gtd_new)) <= -c2 * abs(gtd) and f_new < f) and (c1 > (abs(gtd_new) - abs(gtd))/(f_new - f) > 0 ):
    ...
    while not done  and ls_iter < max_ls and not is_nan:
        ...
        if abs(gtd_new) < abs(gtd) and f_new < f_best and f_new == f_new and c1 > (abs(gtd_new) - abs(gtd))/(f_new - f) > 0:
          success = True
          stall_wolfe = 0
          t_best = t
          f_best = torch.tensor(f_new, device=device)
