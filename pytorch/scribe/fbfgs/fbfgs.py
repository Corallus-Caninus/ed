                  ...
                  while not needle_loss_reduced and needle_norm_order >= 0: # Continue until overall reduction or norm order invalid
                      ...
                      if gtd_at_step_1 < 0 and loss_at_step_1 < best_overall_needle_loss:
                          ...
                          if current_step_t > 1e10: # Arbitrary large number, could be a hyperparameter
                              print(f"    Step size {current_step_t:.4f} exceeded max limit, stopping step increase.") # Indented
                              break # Break inner loop # Indented
                          ...
                          current_loss_at_step, _ = self._directional_evaluate(closure, x_init_needle, current_step_t, d_needle) # Indented
                          # Evaluate loss at the new point
                          # Evaluate loss
                          # Undo step
                          print(f"    Trying step size {current_step_t:.4f} with norm order {needle_norm_order:.2f}, Loss: {current_loss_at_step}") # Indented

                          # Check if this step improved the overall best loss
                          if current_loss_at_step < best_overall_needle_loss: # Indented
                              best_overall_needle_loss = current_loss_at_step # Indented
                              best_overall_d_needle = d_needle.clone() # Store the normalized direction # Indented
                              best_overall_t = current_step_t.clone() # Store the step size # Indented
                              needle_loss_reduced = True # Set overall success flag # Indented
                              # No need to update needle_loss_reduced here, it's already True # Indented

                          # Check the continuation condition: Armijo holds
                          armijo_holds = current_loss_at_step <= loss_baseline_for_step_increase + c1 * current_step_t * gtd_at_step_1 # Indented

                          if armijo_holds: # Indented
                               # Armijo holds, try larger step # Indented
                               current_step_t *= 2 # Increase step size (e.g., double) # Indented
                          else: # Indented
                               # Armijo failed, stop increasing step size for this norm order # Indented
                               print(f"    Armijo failed for norm order {needle_norm_order:.2f}, stopping step increase.") # Indented
                               break # Break inner loop # Indented
                          # --- Inner Loop Ends Here --- # Indented
